...
### Confirmed entity profile URLs and 35,000-character chat ceiling

- Added the owner's exact LinkedIn company and Google Maps URLs to the existing FawazBHSEO ProfessionalService node (an Organization subtype) on Home/About. The existing Person sameAs arrays already matched the two confirmed personal URLs and remain unchanged. All URL strings are preserved verbatim, including the full Maps path.
- No personal alternateName was added; the owner confirmed none exists and that open item is removed. The existing practice alternateName is unaffected. The address, entity IDs, other schema fields, page copy and all protected files remain unchanged. Coordinates present inside the supplied Maps URL were not extracted into geo/latitude/longitude fields.
- Regenerated all three summaries from the final schema: llms.txt 15,361 bytes; llms-full.txt 149,322 bytes; llms-chat.txt 31,280 characters / 31,312 bytes. The private generator now enforces a strict 35,000 ceiling on character, UTF-8 byte and UTF-16 code-unit counts.
- Manual replacements for this change: index.php, about.php, llms.txt, llms-full.txt and llms-chat.txt. README, research and private maintenance tooling are not public upload files. No Hostinger deployment is performed by this repository update.

### AI SEO targeting, brand entities and consolidated citations

This release starts from `8cf5226` on `seo-audit-fixes`. The exact pre-edit titles, descriptions and H1/H2/H3 headings were reported before edits and are preserved in `research/ai-seo-retargeting-report.md`. Query-to-page attribution from independent GSC exports is treated as inferred, not from GSC; no ranking or traffic improvement is asserted.

- `ai-seo-expert-india.php` now leads with AI SEO expert/consultant terminology in its title, description and H1. Its 32 editorial H2/H3 headings are question-shaped, with opening paragraphs of 42-76 words. Shared contact-form UI labels are unchanged. Three new in-page sections explain existing scope, inputs/responsibilities and measurement. No new page, package, price, timeline, client result or deliverable quantity was added.
- The eight existing FAQs and their single generated FAQPage schema are unchanged. Existing factual assertions remain subject to owner review, not revalidated by repository presence. No public review-status notices were restored.
- The homepage title leads with Fawaz BH and its H1 identifies him as an independent SEO consultant. India and Karnataka metadata, H1s and introductory positioning now focus on national SEO/remote delivery and regional SEO. Three distinct introduction links point to the AI SEO page. Existing deeper AEO/GEO capabilities remain.
- Home/About JSON-LD connects the existing Person and ProfessionalService (an Organization subtype) IDs through worksFor/founder references. About includes the same practice node as Home. Existing personal name, jobTitle, LinkedIn/X URLs and full Kushalnagar address remain unchanged. The practice alternateName is the spaced wordmark `Fawaz BH SEO`, already described in the shared logo source. No personal alias, organization social profile, street address or coordinates were invented in that release. The subsequent owner-confirmed profile update is recorded above.
- AEO/GEO now emit one consolidated source line per applicable section: 13 on AEO and 12 on GEO. All anchors and all 22 source records per page remain intact. Existing claims, FAQs and schema are unchanged. The GEO matrix has three substantive columns and five populated rows; the source-only fourth column was removed and its citations consolidated, without inventing a missing value.
- All three text resources were regenerated: `llms.txt` 15,158 bytes; `llms-full.txt` 149,119 bytes; `llms-chat.txt` 31,077 characters / 31,109 bytes. The chat resource remains below 40,000. Maintenance scripts remain private release tooling, not a build step or runtime dependency.
- Validation passed: 54 PHP syntax checks, 33 English and 33 Kannada renders, original anchor preservation, new link/fragment targets, source-register preservation, populated matrix cells, question/word-count checks, FAQ/schema parity and unchanged approved testimonials. Protected-file hashes, canonical/robots/hreflang output and sitemap output are unchanged.
- No changes to `.htaccess`, `includes/i18n.php`, `sitemap.php`, shared includes, any `/api/` path, assets or location pages. No Node/npm/build/deployment machinery was introduced. No Hostinger deployment, live browser/Apache verification or search-result improvement was verified.
- Manual upload set: `index.php`, `ai-seo-expert-india.php`, `seo-consultant-india.php`, `seo-consultant-karnataka.php`, `about.php`, `aeo-consultant.php`, `geo-consultant.php`, `llms.txt`, `llms-full.txt`, `llms-chat.txt`. Keep README, research and `.audit-work` private. Back up the actual server first; the local rollback is not a live-host backup.
- Post-upload: check the four titles/descriptions/H1s and three AI SEO links; inspect question sections and FAQs; validate Home/About entity data; check the three-column engine matrix and consolidated source lines on mobile/desktop; test S01-S22 anchors; verify text resources and chat length; recheck existing HTTPS/non-www routing, locale canonicals and sitemap. Clear stale hosting/CDN caches if necessary.
- Remaining owner decisions: qualification-verification URLs and pre-existing factual-review items. A joined query/page export is needed to verify actual GSC query-to-URL attribution.
...
- Both specialist pages now introduce the service, fit and non-fit, included work, inputs/responsibilities, process, measurement, evidence, scoping, FAQs and contact in that order. Existing mechanism material and the GEO engine matrix sit below the plain-language service and measurement explanation. Original section IDs remain available.
- New handover tables describe only the existing audit, prompt mapping, content/identity and reporting scope. File formats, quantities, revisions, publishing authority, cadence and access boundaries are marked for agreement rather than invented. The measurement example is explicitly an illustration, not a client result.
- All 255 original paragraphs, 14 tested content arrays, original headings/captions/cell wording, FAQ JSON-LD and head metadata are preserved. Existing claims remain pending review; structural completion is not factual clearance.
- Both pages contain the same 22-source register, distinguishing source publication/update dates from observation windows and available sample sizes. There are 112 source-context annotations across the pages. Source links do not validate every surrounding inference. Source dates do not set page review dates, schema dateModified or sitemap dates.
- Private `research/aeo-geo-review-queue.md` contains 18 owner-decision items (R01-R18), with current quotations, findings, proposed wording, sources and explicit NOT APPLIED status. Its 122 quoted passage occurrences include repetition across related issues, not necessarily 122 unique claims. Six additional checks (H01-H06) ask the owner to confirm actual handovers, workflow, scope and evidence. Unverified primary-source attributions are listed rather than silently removed.
- The review covers Search Console reporting/workflow, overlapping AEO/GEO definitions, engine providers and claimed levers, correlation versus causation, study scores/dates/samples, freshness and overlap claims, reporting names, timelines, market rates, schema eligibility and broad retrieval assertions. Current Google help documentation describes the Generative AI performance report/control and an August 2026 rollout; the queue does not incorrectly propose that the feature is nonexistent. The owner's actual usage still needs confirmation.
- Regenerated `llms-full.txt`: 149,561 UTF-8 bytes / 149,161 characters. It retains all eight workstreams, 46 selected research passages/source lines, service FAQs, new service-depth sections, one shared source register, 19 location summaries and three guide summaries. Repeated mechanism/comparison/choosing-provider material is condensed with links. It remains within the previously agreed 100-150 kB range.
- Regenerated `llms-chat.txt`: 29,768 characters / 29,796 UTF-8 bytes, below 40,000. Both resources warn that retained claims await owner review; neither substitutes the proposed corrections. Approved identity, engagements, ILMERA distinction, historical Green Coorg Spices status, testimonial wording and the homepage-derived personal-testing date remain intact. Static snapshots still require explicit regeneration after PHP content changes.
- Validation passed: 54 PHP syntax checks, 33 English and 33 Kannada renders, requested reading order, original text/array preservation, unchanged metadata/canonicals/FAQ schema, eight workstreams, 91 package entries and five approved quote instances. Sitemap output remains identical. These are local PHP/DOM/integrity checks, not live Apache or browser visual verification.
- Unchanged: all backend/API paths, `chat.php`, `llms.txt`, shared includes, assets, i18n, routing and `sitemap.php`. No Node, npm, package file, build step, server, storage service or runtime dependency was added. PHP CLI/Python release tooling is private QA, not a hosting requirement.
- Git status: last fetched remote `main` is `bc61c0150b7578e38411502aa49c186db6f343eb`; this release starts from local `22c08c75f82f172dd62974ed98c3efc7064cb02b`. The pre-release histories have two remote-only and seven local-only commits. Remote main is not an ancestor of local HEAD. No reconciliation, merge, rebase, cherry-pick, remote branch creation, push or PR was performed. A normal local release commit adds one local-only commit; it does not resolve divergence. The requested `.gitignore`/branch/PR work remains deferred pending owner direction.
- Remote `.htaccess` was read using `git show origin/main:.htaccess`; its blob is `c4106347ddda7b8e9db0304aa700487344bf359b`. The file remains absent from this divergent local tree. It was not copied, created, edited or included in the replacement package. Keep the Hostinger version intact.
- Manual upload set, relative to the existing website document root: `aeo-consultant.php`, `geo-consultant.php`, `llms-full.txt`, `llms-chat.txt`. These are complete replacement files, not patches, based on the prior navigation release. README, research, `.audit-work`, decision ledgers, manifests and rollback/review bundles are private and must not be uploaded to the public website.
- Before upload: review R01-R18 and H01-H06, and back up the actual current Hostinger files. This structural draft still publishes retained claims and owner-review notices; use it knowingly as a draft or wait for the factual decisions. The supplied rollback is the local pre-release baseline, not a capture of the current server.
- After an approved manual upload: inspect `/aeo-consultant` and `/geo-consultant` on desktop/mobile, including section order, matrix scrolling, source links, FAQs, navigation, language switching and contact controls. Check both text-resource URLs and sizes; confirm unchanged canonicals/robots and the 32-English-URL `/sitemap.xml`; check for PHP errors and stale caches. No Hostinger upload or live deployment verification was performed.
- Remaining work: owner decisions, approved factual corrections in a separate pass, regenerated snapshots after that pass, live visual/routing verification, and separately authorized resolution of Git divergence before any branch push/PR.
